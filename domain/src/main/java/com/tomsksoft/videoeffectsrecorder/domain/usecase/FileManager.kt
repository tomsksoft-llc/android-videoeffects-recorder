...
class FileManager(private val fileStore: FileStore) {
     ...
     */
    fun create(baseName: String, extension: String): File {
        if (!fileStore.directory.exists())
            fileStore.directory.mkdirs()

        File(fileStore.directory, "$baseName.$extension")
            .takeUnless(File::exists)
            ?.also { return it }

        val sortedTakenIndices = fileStore.getList().stream()
        ...
        var minFreeIndex = 0
        while (minFreeIndex < sortedTakenIndices.size && sortedTakenIndices[minFreeIndex] == minFreeIndex)
            minFreeIndex++

        return File(fileStore.directory, "${baseName}_$minFreeIndex.$extension")
    }
}